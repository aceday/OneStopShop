...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    // Category
        ...
        try {
            $rand_path_uuid = uniqid("product_", true);
            $img_name = $_FILES["update_product_image"]["name"];
            $img_tmp = $_FILES["update_product_image"]["tmp_name"];
            $img_size = $_FILES["update_product_image"]["size"];
            $img_type = $_FILES["update_product_image"]["type"];
            
            $img_type_parse = strtolower(pathinfo($img_name, PATHINFO_EXTENSION));

            $db_media = "/data/image/" . $rand_path_uuid . "." . $img_type_parse;

            if (!in_array($img_type_parse, $image_ext)) {
                echo json_encode(array(
                    "status" => "error",
                    "status_code" => 400,
            ...
            $cur = $pdo->prepare("UPDATE products
                                    SET
                                        ...
                                        product_price_original = :price_original,
